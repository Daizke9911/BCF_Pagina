        ...
        <form method="POST" action="{{route('servicios.store')}}" class="flex flex-col gap-6">
            ...
            <flux:select wire:model="cuentaTypeLogin2" placeholder="Eliga su tipo de cuenta" :label="__('Elija Su Cuenta Para Realizar la Operación')">
               @foreach ($cuentasLogin as $cuentaLogin)
                ...
                <flux:select wire:model="operadora_internet" placeholder="Seleccione operadora" :label="__('Operadora')">
                    <flux:select.option value="Cantv">Cantv</flux:select.option>
                    <flux:select.option value="Fibex">Fibex</flux:select.option>
                    <flux:select.option value="Tecnicable">Tecnicable</flux:select.option>
                    <flux:select.option value="Boom">Boom</flux:select.option>
                    <flux:select.option value="Intel">Intel</flux:select.option>
                </flux:select>
